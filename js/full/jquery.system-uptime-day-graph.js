...
jQuery(document).ready(function($) {
	...
	var options = {
		xaxis: {
			...
			tickSize: [4, "hour"],
			timeformat: "%a</br>%H:%S",
			dayNames: system_uptime_day_graph.day_names
		},
		yaxis: {
			tickDecimals: 0
		},
		...
		grid: {
			color: system_uptime_day_graph.grind_color,
			borderWidth: 1
		},
		legend: {
			show: false
	...
	function getDataAndDraw() {
		$.ajax({
			method: "GET",
			url: system_uptime_day_graph.data_url,
			dataType: "json",
			timeout: 30000,
	...
	function getCacheDataAndDraw() {
		$.ajax({
			method: "GET",
			url: system_uptime_day_graph.cache_data_url,
			dataType: "json",
			timeout: 30000,
			success: function(response) {
				if(response) {
