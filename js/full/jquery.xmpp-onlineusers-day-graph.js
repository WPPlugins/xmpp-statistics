...
jQuery(document).ready(function($) {
	...
	var options = {
		xaxis: {
			...
			tickSize: [4, "hour"],
			timeformat: "%a</br>%H:%S",
			dayNames: xmpp_onlineusers_day_graph.day_names
		},
		yaxis: {
			tickDecimals: 0
		},
		...
		grid: {
			clickable: true,
			hoverable: true,
			color: xmpp_onlineusers_day_graph.grind_color,
			borderWidth: 1
		},
		legend: {
			show: false
	...
	$("#xmpp_onlineusers_day_graph").bind("plothover", function (event, pos, item) {
		...
		if(item) {
			if(previousPoint != item.dataIndex) {
				...
				showToolTip($("#xmpp_onlineusers_day_graph"), item.pageX, item.pageY, item.series.caption + y + xmpp_onlineusers_day_graph.caption + date);
			}
		} else {
			$("#flot-tooltip").remove();
			previousPoint = null;
	...
	function getDataAndDraw() {
		$.ajax({
			method: "GET",
			url: xmpp_onlineusers_day_graph.data_url,
			dataType: "json",
			timeout: 30000,
	...
	function getCacheDataAndDraw() {
		$.ajax({
			method: "GET",
			url: xmpp_onlineusers_day_graph.cache_data_url,
			dataType: "json",
			timeout: 30000,
			success: function(response) {
				if(response) {
