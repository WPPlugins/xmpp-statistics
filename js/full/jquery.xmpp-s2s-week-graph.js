...
jQuery(document).ready(function($) {
	...
	var options = {
		xaxis: {
			...
			tickSize: [1, "day"],
			timeformat: "%a</br>%e.%m",
			dayNames: xmpp_s2s_week_graph.day_names
		},
		yaxis: {
			tickDecimals: 0
		},
		...
		grid: {
			clickable: true,
			hoverable: true,
			color: xmpp_s2s_week_graph.grind_color,
			borderWidth: 1
		},
		legend: {
			show: false
	...
	$("#xmpp_s2s_week_graph").bind("plothover", function (event, pos, item) {
		...
		if(item) {
			if(previousPoint != item.dataIndex) {
				...
				showToolTip($("#xmpp_s2s_week_graph"), item.pageX, item.pageY, y + ' ' + item.series.caption + xmpp_s2s_week_graph.caption + date);
			}
		} else {
			$("#flot-tooltip").remove();
			previousPoint = null;
	...
	function InsertCheckboxes() {
		if($("#xmpp_s2s_week_graph_choices").is(":empty")) {
	...
	function plotAccordingToChoices() {
		//Set custom data
		var custom_data = [];
		$("#xmpp_s2s_week_graph_choices").find("input:checked").each(function () {
		...
		if(custom_data.length > 0) {
			$.plot("#xmpp_s2s_week_graph", custom_data, options);
		}
	}
	...
	function getDataAndDraw() {
		$.ajax({
			method: "GET",
			url: xmpp_s2s_week_graph.data_url,
			dataType: "json",
			timeout: 30000,
	...
	function getCacheDataAndDraw() {
		$.ajax({
			method: "GET",
			url: xmpp_s2s_week_graph.cache_data_url,
			dataType: "json",
			timeout: 30000,
			success: function(response) {
				if(response) {
