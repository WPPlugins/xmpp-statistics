...
jQuery(document).ready(function($) {
	...
	var options = {
		xaxis: {
			...
			tickSize: [1, "day"],
			timeformat: "%a</br>%e.%m",
			dayNames: xmpp_uptime_week_graph.day_names
		},
		yaxis: {
			tickDecimals: 0
		},
		...
		grid: {
			color: xmpp_uptime_week_graph.grind_color,
			borderWidth: 1
		},
		legend: {
			show: false
	...
	function getDataAndDraw() {
		$.ajax({
			method: "GET",
			url: xmpp_uptime_week_graph.data_url,
			dataType: "json",
			timeout: 30000,
	...
	function getCacheDataAndDraw() {
		$.ajax({
			method: "GET",
			url: xmpp_uptime_week_graph.cache_data_url,
			dataType: "json",
			timeout: 30000,
			success: function(response) {
				if(response) {
